<?php
require_once __DIR__ . '/../../../handlers/TrainPersonalScoringHandler.php';
...
// Fetch personal training scores
$stmt = $pdo->prepare('
    SELECT s.score_id, s.training_id, s.training_name, s.m1_score, s.m1_10X, s.m1_109, s.m2_score, s.m2_10X, s.m2_109, 
        s.total_score, s.total_10X, s.total_109, s.event_name, s.event_distance, s.created_at
    FROM personal_training_scores s
    WHERE s.user_id = ? 
...
if (isset($_GET['edit'])) {
    $editMode = true;
    $score_id = $_GET['edit'];
    $stmt = $pdo->prepare('SELECT * FROM personal_training_scores WHERE score_id = ? AND user_id = ?');
    $stmt->execute([$score_id, $user_id]);
    $editScore = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$editScore) {
        $_SESSION['error'] = 'Score not found or permission denied.';
        header('Location: ' . BASE_URL . 'app/views/users/athlete/trainingScoring.php?view=personal');
        exit();
    }
}
?>
    ...
    <div class="accordion-item">
        <h2 class="accordion-header">
            <button class="accordion-button <?php echo $editMode ? '' : 'collapsed'; ?>" type="button" data-bs-toggle="collapse" data-bs-target="#collapseForm" aria-expanded="true" aria-controls="collapseForm">
                <?php echo $editMode ? 'Edit Training Score' : 'Add New Training Score'; ?>
            </button>
        </h2>
                ...
                <form action="" method="POST">
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="training_name" name="training_name" value="<?php echo $editMode ? htmlspecialchars($editScore['training_name']) : ''; ?>" required>
                        </div>

                        <!-- Training ID Input -->
                        <div class="col-md-4 mb-3">
                            <label for="training_id" class="form-label">Training ID</label>
                            <input type="text" class="form-control" id="training_id" name="training_id" value="<?php echo $editMode ? htmlspecialchars($editScore['training_id']) : ''; ?>" required>
                        </div>

                        <!-- Event Name Input -->
                        <div class="col-md-4 mb-3">
                            <label for="event_name" class="form-label">Event Name</label>
                            <input type="text" class="form-control" id="event_name" name="event_name" placeholder="e.g., Recurve U12" value="<?php echo $editMode ? htmlspecialchars($editScore['event_name']) : ''; ?>" required>
                        </div>

                        <!-- Event Distance Input -->
                        <div class="col-md-4 mb-3">
                            <label for="event_distance" class="form-label">Distance (M)</label>
                            <input type="number" class="form-control" id="event_distance" name="event_distance" placeholder="e.g., 50" value="<?php echo $editMode ? htmlspecialchars($editScore['event_distance']) : ''; ?>" required>
                        </div>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            ...
                            <?php if ($editMode): ?>
                                <a href="trainingScoring.php?view=personal" class="btn btn-secondary">Cancel</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </form>
...
<!-- Table displaying personal training scores -->
<div class="table-responsive mb-2">
    <table class="table table-bordered table-striped table-hover shadow-sm">
        ...
        <tbody>
            <?php if (!empty($scores)): ?>
                <?php foreach ($scores as $index => $score): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($score['training_name']); ?></td>
                        ...
                        <td>
                            <div class="d-flex justify-content-center gap-2">
                                <a href="?view=personal&edit=<?php echo htmlspecialchars($score['score_id']); ?>" class="btn btn-warning btn-sm">
                                    <i class="bi bi-pencil"></i> Edit
                                </a>
                                <a href="?view=personal&delete=<?php echo htmlspecialchars($score['score_id']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this score?');">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="15" class="text-center">No personal training scores found.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
