...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE' && isset($_GET['score_id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare('SELECT mareos_id FROM team_training_scores WHERE score_id = ?');
        ...
        // Delete the score
        $stmt = $pdo->prepare('DELETE FROM team_training_scores WHERE score_id = ?');
        $stmt->execute([$score_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($data['athlete_id'], $data['event_name'], $data['event_distance'], $data['m1_score'], $data['m1_10X'], $data['m1_109'], $data['m2_score'], $data['m2_10X'], $data['m2_109'],
        $data['total_score'], $data['total_10X'], $data['total_109'], $data['training_id'])) {
        echo json_encode(['status' => 'error', 'message' => 'Missing required fields']);
        exit();
    }

    try {
        // Check if a score already exists for this athlete and training
        $stmt = $pdo->prepare('SELECT score_id FROM team_training_scores WHERE mareos_id = ? AND training_id = ?');
        $stmt->execute([$data['athlete_id'], $data['training_id']]);
        ...
        if ($existingScore) {
            ...
            $stmt = $pdo->prepare('
                INSERT INTO team_training_scores (user_id, mareos_id, m1_10X, m1_109, m1_score, m2_10X, m2_109, m2_score, total_score, total_10X, total_109, event_name, event_distance, training_id, created_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
            ');
            $stmt->execute([
                $athlete['user_id'], $data['athlete_id'], $data['m1_10X'], $data['m1_109'], $data['m1_score'], $data['m2_10X'], $data['m2_109'], $data['m2_score'],
                $data['total_score'], $data['total_10X'], $data['total_109'], $data['event_name'], $data['event_distance'], $data['training_id']
            ]);
